...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	String imgPath = "http://jiabo.asuscomm.com:12346/";
%>
...
<body>
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-2">
				<img src="<%=imgPath %>${movie.photo}" alt="...">
			</div>
			<div class="col-md-10">
				<h1>${movie.name}</h1>
				<h4>主演：${doubanMovie.casts }</h4>
				<h4>
					评分： <span class="label label-warning">${doubanMovie.rating }</span>
				</h4>
				<h4>类型：${doubanMovie.genres }</h4>
				<h4>国家：${doubanMovie.countries }</h4>
				<h4>年份：${doubanMovie.year }</h4>
				<h4>
					豆瓣连接: <a href="${doubanMovie.alt }" target="_black">
						${doubanMovie.alt }</a>
				</h4>
				<h4>
					简介：
				...
				<div class="btn-group">
					<a onclick="runCmd('${movie.path}');return false;" href="#"
						class="btn btn-primary" role="button">本地播放</a>
					<button type="button" class="btn btn-info" onclick=""
						data-target="#myModal">在线播放</button>
					<a href="movie/video/${movie.id}" class="btn btn-default"
						role="button">下载电影</a>
				</div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-md-12 photos">
				<c:forEach var="photo" items="${photos}" varStatus="s">
					<img alt="" src="<%=imgPath%>${photo.md5}?p=0">
				</c:forEach>
			</div>
		</div>
	</div>
