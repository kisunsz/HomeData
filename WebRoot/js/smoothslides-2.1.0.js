	...
	$.fn.extend({
		smoothSlides: function(options) {
			...
			function captionUpdate() {
				var img=$('#'+uniqueId).find('.ss-slide:eq(-2) img');
				var nextCaption = img.prop('alt');
				if (!nextCaption) {
					$('#'+uniqueId).find(".ss-caption").css('opacity','0');
				} else {
					$('#'+uniqueId).find(".ss-caption").css('opacity','1').html(nextCaption);
					$(".left-title").find("h3").text(nextCaption);
					$(".left-title").find("p").text(img.attr("info"));
				}
			}
			// Captions backward
			...
			if (options.navigation == 'true') {
				$(".right-title").append('<a href="#" class="ss-prev ss-prev-on ">' + options.prevText + '</a><a href="#" class="ss-next ss-next-on">' + options.nextText + '</a>');
			}
			...
			// Navigation Forward
			$('.right-title').on('click', '.ss-next-on', function(event) {
			...
			// Navigation Backward
			$('.right-title').on('click', '.ss-prev-on', function(event) {
			...
			// Disabled nav 
			$('.right-title').on('click', '.ss-prev, .ss-next', function(event) {
				event.preventDefault();
			});
